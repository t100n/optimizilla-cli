        ...
        hasha.fromFile(fileName, { algorithm: 'md5' }).then(hash => {
            ...
            const formData = {
                file: fs.createReadStream(
                    fileName[0] == path.sep ? fileName : path.resolve(process.cwd() + path.sep + fileName)
                ),
                id: randomId,
                name: fileName
            };
...
function downloadFinalFile(body, options, flags) {
    ...
    if(flags.replace) outputFile = options.fileName;
    else outputFile = path.resolve(outputFile + path.sep + body.image.result);
